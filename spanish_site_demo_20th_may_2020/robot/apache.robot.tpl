...
${RUN_SCRIPT}               cd ${RC_SCRIPT_LOCATION}; touch ansible_config_log_1; export ANSIBLE_HOST_KEY_CHECKING=False; /usr/bin/ansible-playbook -i hosts day2-config.yml -e "broker_ip_address=${BROKER_IP_ADDRESS} topic_name=${TOPIC_NAME} metric_id=${METRIC_ID} unit=${UNIT} interval=${INTERVAL} context=${CONTEXT} monitored_file_path=${MONITORED_FILE_PATH} simple=${SIMPLE}" > ansible_config_log_3
